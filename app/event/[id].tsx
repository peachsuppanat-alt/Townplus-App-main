...
const MOCK_EVENTS: any = {
  '1': {
    ...
    tags: ['เทศกาล', 'ของกิน', 'ถ่ายรูปสวย'], lat: 13.7538, lng: 100.5066,
  ...
  '4': {
    ...
    tags: ['อาหาร', 'สตรีทฟู้ด'], lat: 13.7265, lng: 100.5588,
  ...
  '8': {
    ...
    image: 'https://plus.unsplash.com/premium_photo-1663134254080-a3e9f79ae748?q=80&w=1708&auto=format&fit=crop&ixlib=rb-4.1.0&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D',
  ...
  '9': {
    ...
    image: 'https://images.unsplash.com/photo-1622383563227-04401ab4e5ea?q=80&w=774&auto=format&fit=crop&ixlib=rb-4.1.0&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D',
  ...
  '10': {
    ...
    image: 'https://images.unsplash.com/flagged/photo-1562307294-4060df701fa3?q=80&w=1016&auto=format&fit=crop&ixlib=rb-4.1.0&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D',
  ...
  '11': {
    ...
    image: 'https://images.unsplash.com/photo-1768775036854-75341e3a022b?q=80&w=1768&auto=format&fit=crop&ixlib=rb-4.1.0&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D',
  ...
  '12': {
    ...
    image: 'https://images.unsplash.com/photo-1749544292533-65b0ec299191?q=80&w=1750&auto=format&fit=crop&ixlib=rb-4.1.0&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D',
    tags: ['ดนตรี', 'ปิกนิก'], lat: 13.7292, lng: 100.5594,
    adminPicks: [], reviews: []
  }
};
...
export default function EventDetailScreen() {
  ...
  const handleSubmitReview = async () => {
    if (!isLoggedIn) {
      Alert.alert('ต้องเข้าสู่ระบบ', 'คุณต้องเข้าสู่ระบบก่อน จึงจะสามารถให้คะแนนและคอมเมนต์ได้', [
        { text: 'ยกเลิก', style: 'cancel' },
        { text: 'ไปหน้า Login', onPress: () => router.push('/') }
      ]);
      return;
    ...
    Alert.alert('สำเร็จ 🎉', `คุณให้ ${userRating} ดาว!\nและระบบจำลองการส่งข้อมูลให้ Backend แล้ว`);
    setUserRating(0); 
    setReviewText(''); 
  };
        ...
        <View style={styles.contentContainer}>
          <View style={styles.rowBetween}>
            ...
            <View style={styles.ratingBadge}>
              ...
              <Text style={styles.ratingText}>{eventData?.rating} <Text style={styles.reviewText}>({eventData?.reviewsCount})</Text></Text>
            </View>
          </View>
          ...
          {/* Admin Picks */}
          {(eventData?.adminPicks?.length > 0) && (
            <View style={{ marginBottom: 30 }}>
              <View style={{ flexDirection: 'row', alignItems: 'center', marginBottom: 15 }}>
                <Ionicons name="sparkles" size={20} color="#F59E0B" />
                <Text style={[styles.sectionTitle, { marginBottom: 0, marginLeft: 8 }]}>ไฮไลท์แนะนำโดยแอดมิน</Text>
              </View>
              <ScrollView horizontal showsHorizontalScrollIndicator={false} style={{ marginHorizontal: -25, paddingHorizontal: 25 }}>
                {eventData.adminPicks.map((pick: any) => (
                  <View key={pick.id} style={styles.adminPickCard}>
                    <Image source={{ uri: pick.image }} style={styles.adminPickImage} />
                    <View style={styles.adminPickInfo}>
                      <Text style={styles.adminPickTitle} numberOfLines={1}>{pick.title}</Text>
                      <Text style={styles.adminPickDesc} numberOfLines={2}>{pick.desc}</Text>
                    </View>
                  </View>
                ))}
              </ScrollView>
          ...
          <View style={{ marginBottom: 30 }}>
            ...
            {(eventData?.reviews || []).map((rev: any) => (
              <View key={rev.id} style={styles.reviewItem}>
                <View style={styles.reviewHeader}>
                  <View style={styles.reviewAvatar}>
                    <Text style={{ fontFamily: 'Prompt_700Bold', color: '#FFF' }}>{rev.user.charAt(0)}</Text>
                  </View>
...
const styles = StyleSheet.create({
  ...
  ratingText: { fontFamily: 'Prompt_700Bold', color: '#0F172A', fontSize: 14, marginLeft: 4 },
